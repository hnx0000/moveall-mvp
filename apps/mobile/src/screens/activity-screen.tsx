...
const homeGpsSports: SportType[] = ["running", "hiking", "cycling"];
const emptyTrack: MapPoint[] = [];
...
export default function ActivityScreen() {
  ...
  const routineCelebrationPulse = useRef(new Animated.Value(0)).current;
    ...
    useCallback(() => {
      ...
      void Promise.all([
        ...
        .then(async ([servicesEnabled, permission]) => {
          ...
          setGpsReadiness(
            accuracy === null ? "위치 확인됨" : `위치 확인 · 오차 ±${Math.round(accuracy)}m`,
          );
        })
        .catch(() => {
          if (active) {
            setPreviewLocation(undefined);
            setGpsReadiness("시작 시 GPS 재확인");
          }
        });
      return () => {
        active = false;
  ...
  useEffect(() => {
    ...
    routineCelebrationPulse.setValue(0);
    const animation = Animated.parallel([
      Animated.spring(routineCelebrationAnimation, {
        toValue: 1,
        damping: 16,
        stiffness: 190,
        mass: 0.8,
      ...
      createGroovPulseAnimation(routineCelebrationPulse),
    ]);
    animation.start();
    return () => animation.stop();
  }, [completionMessage, routineCelebrationAnimation, routineCelebrationPulse]);
            ...
            <Card style={styles.activityRecordCard}>
              ...
              {showActivityMap ? (
                <View style={styles.activityGpsInfo}>
                  ...
                  <Text style={styles.activityGpsText}>GPS · {activityGpsStatus}</Text>
                </View>
              ) : null}
      ...
      >
        {completionMessage ? (
          <View accessibilityLiveRegion="assertive" style={styles.routineCelebrationBackdrop}>
            <GroovPulseRings progress={routineCelebrationPulse} color={colors.primary} />
            <Animated.View
              style={[
      ...
      {todayRoutines.length ? (
        ...
        <Card style={styles.noRoutineCard}>
          ...
          <Text style={styles.noRoutineText}>
            내 정보에서 운동 종류와 이름, 세부 항목을 정하면 오늘의 루틴에 바로 연결됩니다.
          </Text>
          <PrimaryButton
            label="내 정보에서 루틴 만들기"
...
function createStyles(colors: ThemeColors) {
  return StyleSheet.create({
    ...
    noticeItemRead: { opacity: 0.55 },
    noticeTitle: { color: colors.ink, fontFamily: fonts.bold, fontSize: 12 },
