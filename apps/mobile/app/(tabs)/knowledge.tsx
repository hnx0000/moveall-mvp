...
export default function LeagueScreen() {
  ...
  return (
    <Screen title="">
      ...
      <LinearGradient colors={["#37231C", "#1E1B18", "#121211"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.hero}>
        <View style={styles.heroOrb} />
        <Text style={styles.eyebrow}>SEASON MATCH / PERSONAL</Text>
        <Text style={styles.heroTitle}>{neighborhood}의 순위가{"\n"}<Text style={styles.heroTitleStrong}>오늘도 움직인다.</Text></Text>
        <View style={styles.heroStats}>
          <View style={styles.rankStat}><Text style={styles.statLabel}>MY RANK</Text><View style={styles.rankLine}><Text style={styles.rankValue}>{meta.rank}</Text><Text style={styles.rankTotal}>/ {meta.total}</Text></View></View>
          <View style={styles.scoreStat}><Text style={styles.statLabel}>{meta.label}</Text><Text style={styles.scoreValue}>{meta.score}</Text><Text style={styles.syncText}>운동 기록 자동 반영</Text></View>
        </View>
      ...
      <View style={styles.neighborhoodStats}>
        <View><Text style={styles.neighborhoodStatLabel}>동네 인원</Text><Text style={styles.neighborhoodStatValue}>354명</Text></View>
        <View style={styles.statDivider} />
        <View><Text style={styles.neighborhoodStatLabel}>이번 주 활동</Text><Text style={styles.neighborhoodStatValue}>186명</Text></View>
        <View style={styles.statDivider} />
        <View><Text style={styles.neighborhoodStatLabel}>내 점수</Text><Text style={styles.neighborhoodStatValue}>{meta.score}</Text></View>
      </View>
...
function createStyles(colors: ThemeColors) { return StyleSheet.create({
  ...
  hero: { borderRadius: radius["2xl"], overflow: "hidden", padding: 20, paddingBottom: 0, borderWidth: 1, borderColor: "#583225" }, heroOrb: { position: "absolute", right: -45, top: -42, width: 170, height: 170, borderRadius: 85, backgroundColor: "rgba(255,90,50,.12)", borderWidth: 1, borderColor: "rgba(255,120,75,.25)" }, eyebrow: { color: colors.primary, fontFamily: fonts.displayExtra, fontSize: 9, letterSpacing: 1.25 }, heroTitle: { color: "#F7F3EF", fontFamily: fonts.bold, fontSize: 22, lineHeight: 32, letterSpacing: -1, marginTop: 10 }, heroTitleStrong: { fontSize: 28 }, heroStats: { flexDirection: "row", alignItems: "flex-end", marginTop: 8, marginBottom: 20 }, rankStat: { flex: 1.15 }, scoreStat: { flex: .85, paddingBottom: 8 }, statLabel: { color: "#A9A39D", fontFamily: fonts.displayExtra, fontSize: 8, letterSpacing: 1 }, rankLine: { flexDirection: "row", alignItems: "flex-end" }, rankValue: { color: colors.primary, fontFamily: fonts.displayExtra, fontSize: 92, lineHeight: 96, letterSpacing: -6 }, rankTotal: { color: "#B7B0AA", fontFamily: fonts.display, fontSize: 17, marginBottom: 14, marginLeft: 4 }, scoreValue: { color: "#FFFFFF", fontFamily: fonts.displayExtra, fontSize: 28, marginTop: 8 }, syncText: { color: "#8D8781", fontFamily: fonts.regular, fontSize: 8, marginTop: 3 }, titleBand: { flexDirection: "row", alignItems: "center", gap: 9, backgroundColor: "rgba(255,255,255,.04)", marginHorizontal: -20, paddingHorizontal: 20, paddingVertical: 13, borderTopWidth: 1, borderTopColor: "rgba(255,255,255,.06)" }, titleBandLabel: { color: "#89837E", fontFamily: fonts.medium, fontSize: 8 }, titleBandValue: { color: "#F7F3EF", fontFamily: fonts.bold, fontSize: 10, flex: 1 },
  neighborhoodStats: { flexDirection: "row", alignItems: "center", justifyContent: "space-around", paddingHorizontal: 5 }, neighborhoodStatLabel: { color: colors.muted, fontFamily: fonts.regular, fontSize: 8 }, neighborhoodStatValue: { color: colors.ink, fontFamily: fonts.displayExtra, fontSize: 14, marginTop: 2 }, statDivider: { width: 1, height: 24, backgroundColor: colors.border }, rankingCard: { backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, borderRadius: radius.xl, padding: 15 }, sectionHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-end" }, sectionTitle: { color: colors.ink, fontFamily: fonts.bold, fontSize: 16, marginTop: 2 }, viewAll: { color: colors.muted, fontFamily: fonts.medium, fontSize: 9 }, modeRow: { gap: 17, borderBottomWidth: 1, borderBottomColor: colors.border, marginTop: 9 }, modeButton: { paddingVertical: 10 }, modeButtonActive: { borderBottomWidth: 2, borderBottomColor: colors.primary }, modeText: { color: colors.muted, fontFamily: fonts.medium, fontSize: 10 }, modeTextActive: { color: colors.primary, fontFamily: fonts.bold }, rankRow: { minHeight: 54, flexDirection: "row", alignItems: "center", gap: 9, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: colors.border, paddingHorizontal: 5 }, rankRowMine: { backgroundColor: colors.primarySoft, borderRadius: radius.sm, paddingHorizontal: 10, marginHorizontal: -5 }, rankNumber: { width: 27, color: colors.muted, fontFamily: fonts.display, fontSize: 18 }, mineText: { color: colors.primary }, avatar: { width: 32, height: 32, borderRadius: 16, alignItems: "center", justifyContent: "center", backgroundColor: colors.surfaceMuted }, avatarMine: { backgroundColor: colors.primary }, avatarText: { color: colors.ink, fontFamily: fonts.bold, fontSize: 9 }, avatarTextMine: { color: "#FFFFFF" }, ranker: { flex: 1 }, rankerName: { color: colors.ink, fontFamily: fonts.bold, fontSize: 10 }, rankerMeta: { color: colors.muted, fontFamily: fonts.regular, fontSize: 7, marginTop: 2 }, rankerScore: { color: colors.ink, fontFamily: fonts.displayExtra, fontSize: 16 },
