...
GROOV는 여러 운동을 루틴화하고, 운동 기록을 공유하며, 전문가가 검수한 안전 지식을
...
[가상인물 리스트](<개발자료 보관함/가상인물 리스트/README.md>)에 6명의 설정, 말투, 그록용 프로필·피드 프롬프트와 기존 인물 참고 사진을 함께 정리했습니다.
앱의 데모 피드는 같은 설정 원본을 참조합니다. `pnpm characters:docs`로 문서를 갱신하고,
확정 사진을 넣은 뒤 `pnpm characters:assets`로 앱에 연결합니다. 문구 생성 명령은
[콘텐츠 생성 프로세스](<개발자료 보관함/가상인물 리스트/PROCESS.md>)를 참고하세요.

코드, 운영 문서, 최근 미리보기와 디자인 자료의 전체 위치는
[프로젝트 구조 안내](프로젝트_구조_안내.md)에서 한 번에 확인할 수 있습니다.

## 현재 구현 범위

제품 전체의 사용자 가치는 [핵심 사용자 검증 체크리스트](docs/PRODUCT_VALIDATION.md)로 별도 관리합니다.
순수 기록형·기록+SNS형·경쟁형·개인 성취형의 목적 달성과 만족을 검증하며, 기능 완료율과 합산하지 않습니다.
신규 기능은 네 유형의 수혜·회귀 영향을 역체크합니다.
...
개발 중에는 별도 계정 연결 없이 바로 기능을 확인할 수 있도록 `GROOV 개발자` 세션을 서버가
...
- development: `EXPO_PUBLIC_LOGIN_REQUIRED=true`로 바꾸기 전까지 로그인 없이 내장 MVP 세션 사용
